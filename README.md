# Привет, я Юрий 👋

Строю нейропродукты для бизнеса — сайты, AI-агенты и автоматизации.  
Маркетплейс-эксперт, который однажды устал от ручной рутины и начал всё автоматизировать.

---

## 🔥 Что строю сейчас

| Проект | Описание | Статус |
|---|---|---|
| [НейроЭлектрик](https://t.me/neuro_electric_bot) | Первый в России AI-бот для электриков — текст, фото, голос | 🟡 Тестирование |
| [SH Voice Client](https://t.me/sellershelp_voice_bot) | Клиентский Telegram-бот: аудио/видео → транскрипция с таймкодами + конспект. YouTube, youtube.ru, VK Видео, Дзен, Яндекс Диск и др. | 🟢 Запущен |
| Бот отзывов WB | AI-ответы на отзывы Wildberries с учётом товара и подписи магазина, мультимагазинный режим | 🟡 Тестирование |
| [VoiceTask](https://t.me/voicetaskapp_bot) | Голосовой планировщик задач с AI-разбором и напоминаниями — переработка под MAX | 🔄 Переработка |
| Генератор договоров | MAX-бот: ввёл ИНН → получил готовый DOCX договора за 1 минуту (DaData + python-docx). В планах: счета и акты для действующих клиентов | 🟢 Запущен |
| Экосистема SellersHelp AI | SaaS-инструменты для продавцов WB и Ozon | 🔵 В разработке |

---

## 🚀 Запущено

- 🌐 **[yurazlab.ru](https://yurazlab.ru)** — портфолио и услуги
- 🏢 **[sellershelp.ru](https://sellershelp.ru)** — сайт агентства по маркетплейсам
- 🤖 **[@neuro_electric_bot](https://t.me/neuro_electric_bot)** — AI-ассистент для электриков
- 🎙 **[SH Voice Client](https://t.me/sellershelp_voice_bot)** — клиентский транскрибатор аудио и видео с конспектом
- 📄 **Генератор договоров** — внутренний MAX-бот SellersHelp: ИНН → DOCX за 1 минуту

---

## 🛠 Стек

![Next.js](https://img.shields.io/badge/Next.js-000?style=flat&logo=nextdotjs)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Claude API](https://img.shields.io/badge/Claude_API-D97757?style=flat)
![Telegram](https://img.shields.io/badge/Telegram_Bot-2CA5E0?style=flat&logo=telegram&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000?style=flat&logo=vercel)

---

## 📬 Контакт

- 💬 Telegram: [@yurazlab](https://t.me/yurazlab)
- 📧 Email: yurazlab@gmail.com
- 🌐 Сайт: [yurazlab.ru](https://yurazlab.ru)

---

> *Строю публично — каждый проект это опыт, каждая ошибка — урок.*
